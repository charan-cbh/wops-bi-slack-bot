...
async def generate_sql_with_patterns(user_question: str, user_id: str, channel_id: str) -> Tuple[str, str]:
    """
    Enhanced SQL generation using vector store patterns only (no pattern matcher)
    Returns: (sql_query, selected_table)
    """
    print(f"\n🤖 Starting vector store SQL generation for: {user_question}")

    # Step 1: Use intelligent table discovery via vector store
    ...
    # Step 3: Store the fact that we're generating SQL for context
    ...
    # Step 5: Generate SQL with OpenAI using vector store patterns
    thread_id = await get_or_create_thread(user_id, channel_id)
    if not thread_id:
        return "-- Error: Could not create conversation thread", selected_table

    # Build instructions for vector store-based SQL generation
    instructions = build_vector_store_instructions(
        user_question,
        selected_table,
        schema,
        question_intent
    ...
    # Build message for vector store-based generation
    message = build_vector_store_message(
        user_question,
        selected_table,
        schema,
        question_intent
...
async def enhanced_table_selection(question: str, user_id: str, channel_id: str) -> Tuple[str, str]:
    ...
    # Use vector search for table discovery
    ...
    # Use sampling for final selection - let OpenAI decide based on patterns in vector store
    return await select_best_table_using_samples(question, candidate_tables[:5], user_id, channel_id)
